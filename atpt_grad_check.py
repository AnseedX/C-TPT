...
import matplotlib.pyplot as plt # Import for plotting gradients
import os # Import for file system operations

# Assuming the following imports work based on the provided project structure
try:
...
# --- ORIGINAL FUNCTIONS (KEPT FOR CONTEXT) ---
...
# --- NEW UTILITY FUNCTIONS for Saving/Plotting Gradients ---
def plot_gradients(gradient_norms, set_id, arch, run_type):
    if not gradient_norms:
        print("No gradient data to plot.")
        return

    plt.figure(figsize=(12, 6))
    plt.plot(gradient_norms, label='Gradient L2 Norm (Per Batch)', color='blue', alpha=0.6)
    
    window_size = min(50, len(gradient_norms) // 5)
    if len(gradient_norms) > window_size:
        rolling_mean = np.convolve(gradient_norms, np.ones(window_size)/window_size, mode='valid')
        plt.plot(np.arange(window_size-1, len(gradient_norms)), rolling_mean, 
                 label=f'Rolling Mean (Window {window_size})', color='red', linewidth=2)
        
    plt.title(f'Prompt Gradient L2 Norm on {set_id} ({arch}) - {run_type}')
    plt.xlabel('Batch Index')
    plt.ylabel('Gradient L2 Norm')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
    
    if not os.path.exists('grad_figs'):
        os.makedirs('grad_figs')
    plot_filename = os.path.join('grad_figs', f"gradient_plot_{set_id}_{arch}_{run_type}.png")
    plt.savefig(plot_filename)
    print(f"\n[INFO] Gradient plot saved to {plot_filename}")

def save_gradients_to_csv(gradient_norms, set_id, arch, run_type):
    if not gradient_norms:
        print("No gradient data to save.")
        return

    df = pd.DataFrame({'Gradient_L2_Norm': gradient_norms})
    
    csv_filename = f"gradient_data_{set_id}_{arch}_{run_type}.csv"
    
    if not os.path.exists('grad_data'):
        os.makedirs('grad_data')
    csv_path = os.path.join('grad_data', csv_filename)

    df.to_csv(csv_path, index_label='Batch_Index')
    print(f"[INFO] Gradient data saved to {csv_path}")
    return csv_path
# -------------------------------------------------------------

# --- MODIFIED test_time_tuning to record gradients ---
def test_time_tuning(model, inputs, optimizer, scaler, args, gradient_norms): 
    output = None
    output2 = None
    single_output = None
    if args.cocoop:
    ...
    for j in range(args.tta_steps):
        if 'tpt' in args.run_type or 'atpt' in args.run_type:
        ...
        if args.run_type not in ['baseline', 'baseline_cocoop', 'baseline_coop', 'baseline_ts']:
            ...
            # --- START Gradient Monitoring: Capture L2 Norm ---
            if not args.cocoop and model.prompt_learner.ctx.grad is not None:
                grad_norm = torch.linalg.norm(model.prompt_learner.ctx.grad.float()).item()
                if (j + 1) == args.tta_steps:
...
# --- ORIGINAL main/main_worker (No changes needed) ---
...
def main_worker(gpu, args, result_dict):
    ...
    for set_id in datasets:
        ...
        # MODIFIED: Capture the gradient list
        results_tup, gradient_norms = test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args, result_dict)
        results[set_id] = results_tup
...
# --- MODIFIED test_time_adapt_eval to handle gradient list and perform final output ---
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args, result_dict):
    ...
    gradient_norms = [] # Initialize gradient list
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        if not args.cocoop: 
            ...
            test_time_tuning(model, images, optimizer, scaler, args, gradient_norms)
        else:
            with torch.no_grad():
                with torch.cuda.amp.autocast():
                    image_feature, pgen_ctx = model.gen_ctx(images, args.tpt)
            optimizer = None
            pgen_ctx = test_time_tuning(model, (image_feature, pgen_ctx), optimizer, scaler, args, gradient_norms)
    ...
    # --- POST-PROCESSING: Save to CSV and Plot ---
    save_gradients_to_csv(gradient_norms, args.test_sets, args.arch, args.run_type)
    plot_gradients(gradient_norms, args.test_sets, args.arch, args.run_type)
    # ---------------------------------------------

    return [top1.avg, top5.avg], gradient_norms
